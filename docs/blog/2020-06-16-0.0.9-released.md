...
Recoil 0.0.9 is being released with some bug fixes, [TypeScript support](#typescript-support), and introducing a new API for [Recoil Snapshots](#recoil-snapshots) to observe, inspect, and manage global Recoil atom state.  Thanks again for everyone who helped make this possible and stay tuned for more exciting developments coming soon!

## Bug Fixes

- Some bug fixes for Server Side Rendering and React Native, though we do not officically support them yet. (#108, #233, #220, #284) - Thanks @dustinsoftware, @fyber-LJX, @Chrischuck, and @aulneau
...
TypeScript support is being rolled into the Recoil GitHub repository instead of `DefinitelyTypes` to help better keep it in sync with the API. (#292) - Thanks @csantos42

# Recoil Snapshots

#312, #311, #310, #309, #260, #259, #258, #257, #256 - Thanks @drarmstr and the rest of the team

We are introducing the concept of a [`Snapshot`](/docs/api-reference/core/Snapshot) to Recoil.  A `Snapshot` is an immutable snapshot of the state of Recoil atoms.  This is intended to standardize the API for observing, inspecting, and managing global Recoil state.  It’s useful for dev tools, global state synchronization, history, and navigation.
...
Snapshots are read-only with respect to atom state.  They can be used to read atom state and evaluate selectors derived state.  For asynchronous selectors, the `getPromise()` method can be used to wait for the evaluated value.  Async selectors are also evaluated and can be waited on, so you can see what the selector value would be.
...
Snapshots allow us to observe and synchronize the global state.  But, what if we want a more granular and composable system to work with individual atoms?  We’re working on the concept of *Atom Effects* for observing and dealing with side-effects at the atom level.  This will make it easier to persist state or bi-directionally sync with mutable state.  Think of synchronizing state with the browser URI history.  Coming soon!
