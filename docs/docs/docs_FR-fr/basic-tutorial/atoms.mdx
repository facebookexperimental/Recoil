...
// utilitair pour créer un identifiant unique
let id = 0;
function getId() {
  return id++;
}
...
Notez que nous utilisons la forme **mise à jour** de la fonction s'assignement afin que nous puissions créer une nouvelle liste de tâches basée sur l'ancienne liste de tâches.
