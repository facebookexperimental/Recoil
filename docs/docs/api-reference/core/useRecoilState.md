...
- `state`: an [`atom`](/docs/api-reference/core/atom) or a _writable_ [`selector`](/docs/api-reference/core/selector). writable selectors are selectors that have both a `get` and `set` in their definition while read-only selectors only have a `get`.
