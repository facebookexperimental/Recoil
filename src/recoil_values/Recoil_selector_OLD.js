...
import type {Setter} from '../core/Recoil_GetSetter';
...
const {getNodeLoadable} = require('../core/Recoil_FunctionalCore');
const {
    addToDependencyMap,
...
function selector<T>(
    ...
    function getFromCacheOrEvaluate(
        ...
        const depValues: DepValues = new Map(
            Array.from(currentDeps)
                .sort()
                .map(depKey => {
                    const [deps, loadable] = getNodeLoadable(
                        store,
                        state,
                        depKey,
                    );
    ...
    function evaluateSelectorFunction(
        ...
        try {
            ...
            const result = isRecoilValue(output)
    ...
    if (set != null) {
        // flowlint-next-line unclear-type:off
        (node: any).set = getSetter<T>(set, initSelector);
    }

    return registerNode<T>(node);
}
